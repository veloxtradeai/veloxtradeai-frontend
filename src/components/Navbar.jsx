...
const Navbar = ({ toggleSidebar, isSidebarOpen }) => {
  ...
  const [marketIndices, setMarketIndices] = useState({
    ...
    banknifty: { value: 47520.50, change: '+1.5%', trend: 'up' }
  });
  ...
  // Update market data periodically (simulated)
  useEffect(() => {
    const interval = setInterval(() => {
      setMarketIndices(prev => ({
        nifty: {
          value: prev.nifty.value + (Math.random() * 10 - 5),
          change: `${Math.random() > 0.3 ? '+' : '-'}${(Math.random() * 0.5).toFixed(1)}%`,
          trend: Math.random() > 0.3 ? 'up' : 'down'
        },
        sensex: {
          value: prev.sensex.value + (Math.random() * 20 - 10),
          change: `${Math.random() > 0.4 ? '+' : '-'}${(Math.random() * 0.4).toFixed(1)}%`,
          trend: Math.random() > 0.4 ? 'up' : 'down'
        },
        banknifty: {
          value: prev.banknifty.value + (Math.random() * 15 - 7.5),
          change: `${Math.random() > 0.35 ? '+' : '-'}${(Math.random() * 0.6).toFixed(1)}%`,
          trend: Math.random() > 0.35 ? 'up' : 'down'
        }
      }));
    }, 10000);
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center">
            ...
            <div className="hidden lg:flex items-center space-x-4 ml-6">
              {Object.entries(marketIndices).map(([key, data]) => (
                <div key={key} className="flex items-center space-x-2">
                  <div className="text-xs text-gray-400 uppercase">{key}</div>
                  <div className={`text-sm font-bold ${data.trend === 'up' ? 'text-emerald-400' : 'text-red-400'}`}>
                    {data.value.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </div>
                  <div className={`text-xs px-2 py-0.5 rounded ${data.trend === 'up' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                    {data.change}
                  </div>
                </div>
              ))}
        ...
        <div className="lg:hidden pb-3">
          <div className="flex flex-col space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {Object.entries(marketIndices).slice(0, 3).map(([key, data]) => (
                <div key={key} className="text-center p-2 bg-gradient-to-b from-gray-800/50 to-gray-900/30 rounded-lg border border-emerald-900/20">
                  <div className="text-xs text-emerald-300/70 uppercase">{key}</div>
                  <div className={`text-sm font-bold ${data.trend === 'up' ? 'text-emerald-400' : 'text-red-400'}`}>
                    {data.change}
                  </div>
                </div>
